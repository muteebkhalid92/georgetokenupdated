// presale.js - FIXED VERSION - implements the working presale dapp functionality for the given contract address
...
// FIXED: Improved BNB balance fetching with better error handling and element creation
async function fetchBnbBalance(account, provider) {
  ...
  try {
    ...
    if (bnbBalanceSpan) {
      ...
      console.log("Updated BNB balance span:", bnbBalanceSpan.innerText);
    } else {
      console.warn("Element with id 'bnbBalance' not found");
      // Try to create the element if it doesn't exist
      const displayDiv = document.getElementById("bnbBalanceDisplay");
      if (displayDiv) {
        const span = document.createElement('span');
        span.id = 'bnbBalance';
        span.innerText = parseFloat(bnbBalance).toFixed(4);
        displayDiv.innerHTML = `Your BNB Balance: <span id="bnbBalance">${parseFloat(bnbBalance).toFixed(4)}</span> BNB`;
        console.log("Created missing bnbBalance span");
      }
    }

    if (bnbBalanceDisplay) {
      bnbBalanceDisplay.style.display = "block";
      console.log("Made BNB balance display visible");
    } else {
      console.warn("Element with id 'bnbBalanceDisplay' not found");
      // Try to create the display div if it doesn't exist
      const presaleSection = document.getElementById("presale-section");
      if (presaleSection) {
        const displayDiv = document.createElement('div');
        displayDiv.id = 'bnbBalanceDisplay';
        displayDiv.className = 'text-light mt-3 text-center';
        displayDiv.style.cssText = 'font-weight: bold; font-size: 1.2rem; display: block;';
        displayDiv.innerHTML = `Your BNB Balance: <span id="bnbBalance">${parseFloat(bnbBalance).toFixed(4)}</span> BNB`;
        presaleSection.appendChild(displayDiv);
        console.log("Created missing bnbBalanceDisplay div");
      }
    }
  } catch (error) {
...
// FIXED: Improved buyTokens function with better error handling
async function buyTokens() {
  ...
  try {
    ...
    console.log("Amount:", payInValue);

    // Use signer to send transaction, not contract directly
    const tx = await contract.connect(signer).buyTokens(selectedAccount, {
      value: window.ethers.utils.parseEther(payInValue.toString())
    });
...
// FIXED: Improved updateBnbBalance function
